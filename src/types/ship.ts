...
export interface BaseShipModel {
  ...
  name: string;
  type: string;
  hullClass: 'DDG' | 'FFG' | 'CGN' | 'SSN' | 'BBG' | 'FSG' | 'FAC' | 'BB' | 'HSV' | 'BM';
  description: string;
  hullLength: number;
  hullWidth: number;
  baseHp: number;
...
export interface Particle {
  ...
  size: number;
  color: string;
  type: 'water' | 'smoke' | 'fire' | 'spark' | 'wake' | 'wood' | 'plasma' | 'exhaust' | 'rain' | 'snow' | 'ember';
}
...
export interface BattleIsland {
  ...
  foliage: { x: number; y: number; radius: number; color: string; type?: 'tree' | 'ice' | 'bunker' | 'buoy' | 'vent' }[];
...
export type GameMode = 'fleet-battle' | 'area-control';

export interface ObjectiveZone {
  id: string;
  name: string;
  teamOwner: Team; // 'player' for Omega (Blue), 'enemy' for Alpha (Red)
  x: number;
  y: number;
  radius: number;
  color: string;
...
export interface ObjectiveZones {
  omegaZone: ObjectiveZone; // Sector Omega (Blue, Player home base on west)
  alphaZone: ObjectiveZone; // Sector Alpha (Red, Enemy home base on east)
}

export interface CaptureProgress {
  // Sector Alpha (Red, Hostile Base in the East): Captured by Player team
  alphaCapturePercent: number; // 0 to 100% (at 100% player wins)
  alphaStatus: 'idle' | 'capturing' | 'contested';
  alliesInAlpha: number;
  enemiesInAlpha: number;

  // Sector Omega (Blue, Allied Base in the West): Captured by Enemy team
  omegaCapturePercent: number; // 0 to 100% (at 100% enemy wins)
  omegaStatus: 'idle' | 'capturing' | 'contested';
  alliesInOmega: number;
  enemiesInOmega: number;
}
...
export interface BattleSettings {
  ...
  gameMode: GameMode; // 'fleet-battle' | 'area-control'
}
