...
import { BaseShipModel, CustomShipConfig, GameMode, SavedShipProfile } from '../types/ship';
...
const COLOR_PALETTES = [
  { name: 'Naval Haze Gray', primary: '#475569', accent: '#64748b' },
  { name: 'Battleship Dark Slate', primary: '#334155', accent: '#475569' },
  { name: 'Maritime Ocean Blue', primary: '#243342', accent: '#3b4e61' },
  { name: 'Littoral Olive Camo', primary: '#3f4a3c', accent: '#52624e' },
  { name: 'Teak Deck & Red Oxide', primary: '#5c2d28', accent: '#38424d' },
  { name: 'Anechoic Submarine Black', primary: '#292524', accent: '#44403c' },
  { name: 'Arctic Low-Vis Gray', primary: '#64748b', accent: '#94a3b8' },
  { name: 'Coastal Sandstone Buff', primary: '#574635', accent: '#715c46' },
];
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 lg:p-8 flex flex-col gap-6">
        ...
        <section className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 shadow-xl">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-100">Select Mission Objective & Game Mode</h3>
            </div>
            <span className="text-[11px] font-mono text-cyan-400 bg-cyan-950/80 px-2 py-0.5 rounded border border-cyan-800/60">
              {gameMode === 'area-control' ? 'Area Control Active' : 'Normal Battle Active'}
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Mode 1: Fleet Annihilation */}
            <div
              onClick={() => {
              ...
              className={`p-4 rounded-xl border transition-all cursor-pointer flex flex-col justify-between ${
                gameMode === 'fleet-battle'
                  ? 'bg-cyan-950/40 border-cyan-500 shadow-lg shadow-cyan-950/50 ring-1 ring-cyan-400'
                  : 'bg-slate-800/40 border-slate-700/60 hover:bg-slate-800/80 hover:border-slate-600'
              }`}
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-lg ${gameMode === 'fleet-battle' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-400'}`}>
                      <Swords className="w-4 h-4" />
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-slate-100">Fleet Annihilation</h4>
                      <span className="text-[10px] text-cyan-400 font-mono">Normal Battle Mode</span>
                    </div>
                  </div>
                  {gameMode === 'fleet-battle' && (
                    <span className="text-[10px] font-mono font-bold bg-cyan-500 text-slate-950 px-2 py-0.5 rounded-full">
                      SELECTED
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Classic naval warfare. Maneuver across the battlefield, coordinate salvoes, and sink every enemy warship. Victory goes to the last surviving fleet afloat.
                </p>
              </div>

              <div className="mt-3 pt-2.5 border-t border-slate-700/60 flex items-center justify-between text-[10px] font-mono text-slate-400">
                <span>Objective: Total Fleet Destruction</span>
                <span className="text-cyan-400">100% Sunk = Win</span>
              </div>
            </div>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-lg ${gameMode === 'area-control' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-400'}`}>
                      <Flag className="w-4 h-4" />
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-slate-100">Area Control</h4>
                      <span className="text-[10px] text-amber-400 font-mono">Infiltration & Sector Hold</span>
                    </div>
                  </div>
                  {gameMode === 'area-control' && (
                    <span className="text-[10px] font-mono font-bold bg-cyan-500 text-slate-950 px-2 py-0.5 rounded-full">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Enter and hold the designated strategic zone on the enemy side (<span className="text-cyan-400 font-medium">Sector Omega</span>) for a set time to win. Defend your home base (<span className="text-rose-400 font-medium">Sector Alpha</span>) from hostile capture!
                </p>
              </div>

              <div className="mt-3 pt-2.5 border-t border-slate-700/60 flex items-center justify-between text-[10px] font-mono text-slate-400">
                <span>Objective: Hold Enemy Zone for 35s</span>
                <span className="text-amber-400">Dual Win Condition</span>
              </div>
            </div>
          </div>
        </section>

        {/* Step 5: Fleet Battle Configuration & Launch Banner */}
        <section className="bg-gradient-to-r from-slate-900 via-slate-900/95 to-cyan-950/40 border border-slate-800 rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xl">
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-cyan-600/30 text-cyan-400 text-xs flex items-center justify-center font-bold">5</span>
          ...
          <div className="w-full md:w-auto flex flex-col items-end gap-2">
            ...
            >
              ...
              <span>
                Launch {gameMode === 'area-control' ? 'Area Control' : 'Fleet Battle'} ({shipsPerTeam}v{shipsPerTeam})
              </span>
            </button>
            <span className="text-[11px] text-slate-400 font-mono">
              {gameMode === 'area-control' ? 'Capture & Hold Enemy Sector Omega' : 'Annihilate All Enemy Warships'}
            </span>
          </div>
        </section>
