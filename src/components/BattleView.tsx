...
export const BattleView: React.FC<BattleViewProps> = ({
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-slate-950 select-none">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1.5">
              ...
              <span className={`text-[9px] font-mono px-1.5 py-0.2 rounded font-bold ${
                ...
                {battleState?.gameMode === 'area-control' ? 'AREA CONTROL' : 'FLEET BATTLE'}
              </span>
            </div>
          </div>
      ...
      {/* Area Control Tactical Objective HUD (Infiltration & Sector Hold) */}
      {battleState?.gameMode === 'area-control' && battleState?.captureProgress && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-20 pointer-events-auto flex items-center gap-3 sm:gap-6 bg-slate-900/95 backdrop-blur-md px-4 sm:px-6 py-2.5 rounded-2xl border border-slate-700/80 shadow-2xl">
          {/* Sector Omega (Allied Bastion - Blue Team / Player Fleet) */}
          <div className="flex items-center gap-2.5">
            <div className="text-right">
              <div className="flex items-center justify-end gap-1.5 text-xs font-bold text-slate-100">
              ...
              <div className="text-[10px] font-mono mt-0.5">
                {battleState.captureProgress.omegaStatus === 'capturing' ? (
                  <span className="text-rose-400 font-bold animate-pulse">UNDER ASSAULT!</span>
                ) : battleState.captureProgress.omegaStatus === 'contested' ? (
                  <span className="text-amber-400 font-bold">CONTESTED</span>
                ) : (
                  <span className="text-slate-400">
                    SECURE ({battleState.captureProgress.enemiesInOmega} Hostiles)
                  </span>
                )}
              </div>
            </div>
            <div className="w-16 sm:w-28 h-3 bg-slate-800 rounded-full border border-slate-700 overflow-hidden relative">
              <div
                className={`h-full transition-all duration-200 ${
                  battleState.captureProgress.omegaCapturePercent > 50 ? 'bg-rose-500 animate-pulse' : 'bg-cyan-500'
                }`}
                style={{ width: `${Math.min(100, Math.max(0, battleState.captureProgress.omegaCapturePercent))}%` }}
              />
            </div>
            <span className="text-xs font-mono font-bold text-cyan-400 w-10 text-left">
              {Math.round(battleState.captureProgress.omegaCapturePercent)}%
            </span>
          </div>

          <div className="h-7 w-px bg-slate-700/80" />

          {/* Sector Alpha (Hostile Redoubt - Red Team / Enemy Fleet) */}
          <div className="flex items-center gap-2.5">
            <span className="text-xs font-mono font-bold text-rose-400 w-10 text-right">
              {Math.round(battleState.captureProgress.alphaCapturePercent)}%
            </span>
            <div className="w-16 sm:w-28 h-3 bg-slate-800 rounded-full border border-slate-700 overflow-hidden relative">
              <div
                className={`h-full transition-all duration-200 ${
                  battleState.captureProgress.alphaCapturePercent > 65 ? 'bg-rose-500 animate-pulse' : 'bg-rose-600'
                }`}
                style={{ width: `${Math.min(100, Math.max(0, battleState.captureProgress.alphaCapturePercent))}%` }}
              />
            </div>
            <div className="text-left">
              ...
              <div className="text-[10px] font-mono mt-0.5">
                {battleState.captureProgress.alphaStatus === 'capturing' ? (
                  <span className="text-cyan-400 font-bold animate-pulse">
                    HOLDING (+{battleState.captureProgress.alliesInAlpha})
                  </span>
                ) : battleState.captureProgress.alphaStatus === 'contested' ? (
                  <span className="text-amber-400 font-bold">CONTESTED</span>
                ) : (
                  <span className="text-slate-400">INFILTRATE ENEMY SECTOR</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="absolute inset-0 z-40 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 lg:p-8 max-w-md w-full shadow-2xl text-center flex flex-col items-center">
            ...
            <h2 className="text-2xl font-black text-slate-100 tracking-tight">
              {battleState.winner === 'player'
                ? battleState.winReason === 'area_control'
                  ? 'SECTOR CONTROL VICTORY!'
                  : 'FLEET ANNIHILATION VICTORY!'
                : battleState.winReason === 'area_control'
                ? 'SECTOR OVERRUN!'
                : 'FLEET DESTROYED'}
            </h2>
            <p className="text-xs text-slate-300 mt-1 mb-6 leading-relaxed">
              {battleState.winner === 'player'
                ? battleState.winReason === 'area_control'
                  ? 'Allied forces penetrated enemy waters and held Sector Omega for the required duration! Tactical dominance achieved.'
                  : 'All hostile NPC warships have been eliminated. Your modern warship design proved completely victorious!'
                : battleState.winReason === 'area_control'
                ? 'Hostile fleet penetrated and captured Sector Alpha while allied defenses fell.'
        ...
        <div className="absolute inset-0 z-40 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex flex-col gap-2.5 my-4 text-xs">
              ...
              <div className="p-2 rounded-lg bg-cyan-950/60 border border-cyan-800/40 text-[11px] text-cyan-200">
                <span className="font-bold text-cyan-300 block mb-0.5">Area Control Rules:</span>
                Steer into enemy Sector Omega to capture and hold it to 100% while intercepting enemies attempting to take Sector Alpha.
              </div>
            </div>
...
function renderMinimap(canvas: HTMLCanvasElement, state: BattleState) {
  ...
  // Islands (Draw exact polygon boundaries on minimap radar)
  ...
  // Objective Zones (in Area Control mode)
  if (state.gameMode === 'area-control' && state.objectiveZones) {
    const { omegaZone, alphaZone } = state.objectiveZones;

    // Sector Omega (Blue / Allied Base in the West)
    ctx.save();
    ctx.strokeStyle = '#38bdf8';
    ctx.lineWidth = 1.5;
    ctx.fillStyle = 'rgba(56, 189, 248, 0.25)';
    ctx.beginPath();
    ctx.arc(omegaZone.x * scaleX, omegaZone.y * scaleY, omegaZone.radius * scaleX, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    // Sector Alpha (Red / Hostile Redoubt in the East)
    ctx.strokeStyle = '#f87171';
    ctx.lineWidth = 1.5;
    ctx.fillStyle = 'rgba(248, 113, 113, 0.25)';
    ctx.beginPath();
    ctx.arc(alphaZone.x * scaleX, alphaZone.y * scaleY, alphaZone.radius * scaleX, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.restore();
  }
